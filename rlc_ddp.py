...
if __name__ == '__main__':
    ...
    next_state = jit(lambda x, u: rlc.next_states(x, u))  # x(i+1) = f(x(i), u)
    running_cost = jit(lambda x, u: 0.5 *500* jnp.sum(jnp.square(u))) # l(x, u), multiplied with 0.2 to reduce importance
    final_cost = jit(lambda x: 0.5 * (10*jnp.square(1 - x[1])) + 500*jnp.square(x[0])) # lf(x)

    ddp = DDP(next_state, running_cost, final_cost, rlc.get_state_dim(), pred_time=100)
